...
class HssOktaFlutterPlugin : HssOktaFlutterPluginApi, FlutterPlugin, ActivityAware {
    ...
    private val browserSignInChannelName =
            "dev.hypersense.software.hss_okta.channels.browser_signin"
    private val browserSignOutChannelName =
    ...
    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        ...
        browserSignOutEventChannel =
                EventChannel(binding.binaryMessenger, browserSignOutChannelName)
    ...
    override fun startDeviceAuthorizationFlow(
            callback: (Result<DeviceAuthorizationSession?>) -> Unit
    ) {
        try {
            CoroutineScope(Dispatchers.IO).launch {
                deviceAuthorizationFlow =
                        CredentialBootstrap.oidcClient.createDeviceAuthorizationFlow()
    ...
    override fun startTokenExchangeFlow(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val tokenExchangeFlow: TokenExchangeFlow =
                    CredentialBootstrap.oidcClient.createTokenExchangeFlow()
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val selectedCredential =
                        credentialList.firstOrNull { credential ->
                            credential.token?.idToken == tokenId
                        }

                if (selectedCredential != null) {
                    callback.invoke(
                            Result.success(
                                    selectedCredential.token?.let {
                                        composeOktaResult(
                                                res = it,
                                                userInfoResult =
                                                        selectedCredential
                ...
                callback.invoke(
                        Result.failure(HssOktaFlutterException(code = "Error", "No token found"))
                )
            } catch (exception: Exception) {
                callback.invoke(
                        Result.failure(
    ...
    override fun setDefaultToken(tokenId: String, callback: (Result<Boolean>) -> Unit) {
        throw HssOktaFlutterException(
                code = "Missing",
                message = "This is not available for Android"
        )
    }

    override fun startBrowserSignin(callback: (Result<AuthenticationResult>) -> Unit) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            when (val result =
                            webAuthenticationClient.login(
                                    context!!,
                                    "${BuildConfig.SIGN_IN_REDIRECT_URI}"
                            )
            ) {
                is OidcClientResult.Error -> {
                    callback.invoke(
                ...
                is OidcClientResult.Success -> {
                    ...
                    callback.invoke(
                            Result.success(
                                    composeOktaResult(
                                            userInfoResult = credential.getUserInfo().getOrThrow(),
    ...
    override fun startBrowserSignout(callback: (Result<Boolean>) -> Unit) {
